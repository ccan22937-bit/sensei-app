...
class LiteRTLMEngine(private val context: Context) {
    ...
    /**
     * Resolves the active model file in Sensei's private app directory
     */
    fun getActiveModelFile(): File {
        val primary = File(context.filesDir, "models/$PRIMARY_MODEL_FILENAME")
        if (primary.exists() && primary.length() >= MIN_VALID_MODEL_SIZE_BYTES) {
            return primary
        }
        val alt = File(context.filesDir, "models/$ALT_MODEL_FILENAME")
        if (alt.exists() && alt.length() >= MIN_VALID_MODEL_SIZE_BYTES) {
            return alt
        }
        return primary
    }
